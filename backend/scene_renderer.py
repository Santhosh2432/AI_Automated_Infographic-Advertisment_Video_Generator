...
def _draw_particles(draw: ImageDraw.Draw, accent: tuple, alpha: int, frame_idx: int):
    """Draw subtle floating particles in the background."""
    for i in range(12):
        # Deterministic but seemingly random positions
        seed = i * 100
        x_base = (seed * 17) % WIDTH
        y_base = (seed * 23) % HEIGHT
        
        # Slow drift based on frame_idx
        dx = 40 * math.sin(frame_idx * 0.01 + i)
        dy = 40 * math.cos(frame_idx * 0.015 + i)
        
        x, y = x_base + dx, y_base + dy
        r = 2 + (i % 3)
        # alpha is from the scene progress, so we fade particles in/out
        p_alpha = int((alpha // 10) * (0.5 + 0.5 * math.sin(frame_idx * 0.02 + i)))
        
        draw.ellipse([x - r, y - r, x + r, y + r], fill=_rgba(accent, p_alpha))
...
def _draw_scene_frame(scene: dict, progress: float, frame_idx: int,
    ...
    _draw_particles(draw, accent, deco_alpha, frame_idx)
    ...
    num_bullets = len(bullets[:MAX_BULLETS])
    current_bullet_size = FONT_BULLET_SIZE
    current_bullet_spacing = BULLET_SPACING
    
    if num_bullets > 5:
        # Shrink slightly to fit many points elegantly
        current_bullet_size = 34
        current_bullet_spacing = 62
        
    bullet_font = _get_font(current_bullet_size)
    bullet_start_y = 280
    stagger = min(0.12, 0.6 / max(1, num_bullets))

    for i, bullet_text in enumerate(bullets[:MAX_BULLETS]):
        # Each bullet starts animating at (25% + i * stagger) of scene progress
        b_start = 0.25 + i * stagger
        ...
        b_y = bullet_start_y + i * current_bullet_spacing

        # Bullet dot
        dot_r = 6
        draw.ellipse(
            [b_x - 25, b_y + (current_bullet_size // 2 - dot_r), 
             b_x - 25 + dot_r * 2, b_y + (current_bullet_size // 2 + dot_r)],
